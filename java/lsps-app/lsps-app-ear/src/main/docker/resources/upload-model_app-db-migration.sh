...
echo "-             Upload Model             -"
echo "----------------------------------------"
if java -jar $JBOSS_HOME/lsps/lsps-cli-*-full.jar modelUpload -h http://localhost:8080 -m $JBOSS_HOME/lsps/*.zip -u admin -p admin --dbUpdateStrategy update; then
...
echo "----------------------------------------"
if java -jar $JBOSS_HOME/lsps/lsps-cli-*-full.jar updateSetting -h http://localhost:8080 -u admin -p admin -n ENABLE_DROP_CREATE -v true; then
